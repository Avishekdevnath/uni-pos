...
import type { CreateStockInInput } from '@/types/inventory';
...
interface StockInFormProps {
  onSubmit: (data: CreateStockInInput) => Promise<unknown>;
  isSubmitting: boolean;
  branchId: string;
}

export function StockInForm({ onSubmit, isSubmitting, branchId }: StockInFormProps) {
  ...
  const { register, handleSubmit, setValue, control, watch, formState: { errors } } = useForm<FormData>({
    ...
    defaultValues: { items: [{ product_id: '', quantity: 1, unit_cost: 0 }] },
  });
          ...
          return (
            <div key={field.id} className="flex gap-3 items-end">
              ...
              <div className="w-28 space-y-1">
                ...
                <Input type="number" step="0.01" min={0} {...register(`items.${index}.unit_cost`)} />
              </div>
              <Button
                type="button"
                variant="ghost"
